...
# the following generates a uniform hypergraph
...
# the following takes a list of hyperedges as input; group-based dynamics
function montecarlo_qs(edges, δ, ξ, ν, ϵ, max_timesteps, Δt, average_window, M, update_p)
  L = length(edges)
  N = length(unique(Iterators.flatten(edges)))
...
# same but for node-based dynamics
function montecarlo_qs(edges, δ, ξ, ν, ϵ, max_timesteps, Δt, average_window, M, update_p)
  L = length(edges)
  N = length(unique(Iterators.flatten(edges)))
